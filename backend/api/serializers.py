...
from .models import Ingredient, Recipe, Tag, RecipeIngredient, Fav, ShoppingList, Subscription
...
class RecipeListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Recipe
        fields = ('id', 'name', 'author', 'ingredients', 'image', 'tags', 'is_favorited', 'is_in_shopping_cart', 'text', 'cooking_time',)
    ...
    def get_is_favorited(self, obj):
        return self.request_user.is_authenticated and Fav.objects.filter(user=self.request_user).exists()

    def get_is_in_shopping_cart(self, obj):
        return self.request_user.is_authenticated and ShoppingList.objects.filter(user=self.request_user, recipe=obj).exists()
...
class RecipeCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if len(ingredient_ids) != len(set(ingredient_ids)):
            raise serializers.ValidationError('Ингредиенты в рецепте повторяются.')
        
        if not Ingredient.objects.filter(id__in=ingredient_ids).exists():
            raise serializers.ValidationError(
                f'Ингредиент не существует.'
            )
        tags = data.get('tags')
        if not tags:
...
class FavoriteSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if request:
            ...
            return MiniRecipeSerializer(instance.recipe, context=context).data
